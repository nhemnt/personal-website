...
class ColorSwitcher extends Component {
  ...
  getClassName = color => color.split('_')[1].toLowerCase()
      ...
      <div className={`theme-colors ${this.state.isOpen ? 'shown' : ''}`}>
        <div className="p-4">
          ...
          <div className="d-flex flex-row justify-content-between mb-4">
            {Object.keys(colors)
              .filter(color => color.includes('LIGHT'))
              .map(color => (
                <button
                  key={`light.${color}`}
                  className={`theme-color theme-color-${this.getClassName(
                    color
                  )} ${selectedColor === `light.${color}` ? 'active' : ''}`}
                  style={{ background: '#fff' }}
                  onClick={e => this.changeThemeColor(e, `${color}`)}
                >
        ...
        <a href="#section" className="theme-button" onClick={this.toggle}>
          ...
          <svg
            ...
            data-prefix="fas"
            data-icon="cog"
            className="svg-inline--fa fa-cog fa-w-16 fa-spin"
